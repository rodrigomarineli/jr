...
$html .= '<tr>';
	...
	$html .= '<td><b>'.utf8_decode('Data do Orçamento').'</b></td>';
	$html .= '<td><b>'.utf8_decode('Cliente').'</b></td>';
	$html .= '<td><b>'.utf8_decode('Tipo').'</b></td>';
	$html .= '<td><b>'.utf8_decode('Produto').'</b></td>';
	$html .= '<td><b>'.utf8_decode('Quantidade').'</b></td>';
	$html .= '<td><b>'.utf8_decode('Finalidade').'</b></td>';
$html .= '</tr>';
foreach ($user['dados'] as $lista) {
	$cliente = CRUD::SelectOne('cliente','id',$lista['id_cliente']);
	
	$itens = CRUD::SelectOne('pedidos_produtos','id_pedidos',$lista['id']);
	foreach ($itens['dados'] as $lista_itens) {
		if($lista_itens['tipo'] == 1) {
			$produto = CRUD::SelectOne('produtos','id',$lista_itens['id_produto']);
			$tipo = "Produtos";
		}
		else if($lista_itens['tipo'] == 2) {
			$produto = CRUD::SelectOne('pecas','id',$lista_itens['id_produto']);
		...
		$html .= '<tr>';
			...
			$html .= '<td>'.utf8_decode($produto['dados'][0]['nome']).'</td>';
			$html .= '<td>'.utf8_decode($lista_itens['qtd']).'</td>';
			if($lista_itens['modo'] == 1) {
				$html .= '<td>'.utf8_decode('Compra').'</td>';
			} else if($lista_itens['modo'] == 2) {
				$html .= '<td>'.utf8_decode(date('d/m/Y', strtotime($lista_itens['de'])).' até '.date('d/m/Y', strtotime($lista_itens['ate']))).'</td>';
			}
		$html .= '</tr>';
		$cont++;
	}
