											...
											<li>
												<div class="sort-by">
													...
													<div class="select-box inline-block">
														<select class="qtdPag">
															<option <?php if($limit == 12) echo 'selected'; ?> value="<?php echo URLBASE.'busca/'.$urlFiltro.$s.'/limit/12' ?>">12</option>
															<option <?php if($limit == 16) echo 'selected'; ?> value="<?php echo URLBASE.'busca/'.$urlFiltro.$s.'/limit/16' ?>">16</option>
															<option <?php if($limit == 20) echo 'selected'; ?> value="<?php echo URLBASE.'busca/'.$urlFiltro.$s.'/limit/20' ?>">20</option>
															<option <?php if($limit == 24) echo 'selected'; ?> value="<?php echo URLBASE.'busca/'.$urlFiltro.$s.'/limit/24' ?>">24</option>
														</select>
													</div>
												</div>
											</li>
							...
							<div class="product-list-view">
								<div class="row">
									<div class="col-md-12">
										<?php
											if(isset($limit)) {
												$urlPag = URLBASE.'busca/'.$urlFiltro.$s.'/limit/'.$limit;
											} else {
												$urlPag = URLBASE.'busca/'.$s;
											}

											$busca = "'%$s%'";
											$sql = "SELECT *, 1 AS tipo FROM produtos AS A WHERE nome LIKE $busca OR descricao LIKE $busca OR `especificacoes` LIKE $busca UNION SELECT *, 2 AS tipo FROM pecas AS B WHERE nome LIKE $busca OR descricao LIKE $busca OR `especificacoes` LIKE $busca";
												...
												<div class="col-md-8 col-sm-8 col-xs-12">
													<div class="product-info">
														<div class="table-custom title12">
															<div class="text-left">
																<a href="<?php echo URLBASE.$linkProduto.$listaProdutos['url'] ?>" class="cat-parent title16 font-bold dark text-uppercase"><?php echo $listaProdutos['nome'] ?></a>
															</div>
														</div>
														<br/>
														<br/>
														<form method="post" action="<?php echo URLBASE.'carrinho' ?>">
															<ul class="wrap-qty-cart list-inline-block pull-left">
																...
																<li>
																	...
																	<input type="hidden" name="tipo" value="<?php echo $listaProdutos['tipo'] ?>">
														...
														<a href="<?php echo URLBASE.$linkProduto.$listaProdutos['url'] ?>" class="shop-button dark">SAIBA MAIS</a>
													</div>	
												</div>
											</div>
										</div>
